...
| `qa` | Reviewer-triggered functional QA — verifies a code-reviewed, merged change works as expected. QAs subtasks one at a time, runs the feature's existing tests (stack-discovered, not just `package.json`), generates/runs Playwright e2e for web AC and routes non-web (mobile/backend/config) to the existing suite or an AC-derived manual checklist, writes `qa.md` (`pending`/PASS/FAIL/BLOCKED — BLOCKED only when the AC can't be exercised at all), offers opt-in `qa.html`, lists manual checks. On a PASS against a ticket, offers (opt-in) to post the QA summary as a ticket comment and move it to the team's ready-for-release status. Tests against the AC; does not back-engineer the feature |
...
| `/casaflow:qa` | `core/skills/qa/SKILL.md` | Reviewer-triggered functional QA: does the feature work as expected with the current code (already code-reviewed + merged to dev)? NOT a second code review, and does NOT back-engineer the feature from the diff. Resolve target (ticket/PR/branch) → if the ticket has subtasks, QA them one at a time → read AC (+ spec.md if present) → run the feature's existing tests (discover the stack's command, fall back to CI; not just `package.json`) → for web changes discover Playwright and run existing e2e or generate happy-path specs for the AC (reuse repo auth fixture; route to manual if none), for non-web route to the existing suite or an AC-derived manual checklist → write `qa.md` with `result: pending|PASS|FAIL|BLOCKED` (pending = manual checks await sign-off; BLOCKED only when the AC can't be exercised by any form) → required opt-in `qa.html` prompt → list manual checks (each traces to an AC item, signed off individually) → on PASS against a ticket, offer (opt-in) to post the QA summary as a ticket comment and transition it to the team's ready-for-release status (confirm target; never automatic). Does not derive backend check matrices, mint per-role auth, promote regression branches/PRs, spawn bug tickets, or auto-post reviews; the only tracker write is the reviewer-confirmed PASS comment + transition. |
| `/casaflow:pr-review-swarm` | `core/skills/pr-review-swarm/SKILL.md` | Reviewer-triggered end-to-end PR review (you've been added as reviewer). Resolve PR (url/#/branch) → `gh pr checkout` → CI-status gate (`gh pr checks`: failing = changes, pending = hold) → scope check vs Jira ticket/spec AC → coding-guideline check → dispatch `review` swarm `tier: all`, then verify each finding for correctness before posting (drop false positives) → VERDICT. Changes/red-CI → REQUEST_CHANGES via `pr-review` agent (confirm) → STOP. Approve-worthy → mandatory QA via `qa` skill, **form adapts to PR type** (web e2e / mobile manual checklist / backend suite / config build-and-check); BLOCKED only means "couldn't QA at all", not "no Playwright harness". On QA PASS → offer to land coverage (opt-in; follow-up PR for forks/protected branches) → post APPROVE (confirm) → offer merge (confirm). NEVER posts/pushes/approves/merges without confirmation; NEVER approves before QA passes or over red CI. |
