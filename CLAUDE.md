...
| `qa` | Reviewer-triggered functional QA — verifies a code-reviewed, merged change works as expected. QAs subtasks one at a time, runs the feature's existing tests (stack-discovered, not just `package.json`), generates/runs Playwright e2e for web AC and routes non-web (mobile/backend/config) to the existing suite or an AC-derived manual checklist, writes `qa.md` (`pending`/PASS/FAIL/BLOCKED — BLOCKED only when the AC can't be exercised at all), offers opt-in `qa.html`, lists manual checks. Tests against the AC; does not back-engineer the feature |
